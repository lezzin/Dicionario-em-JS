...
const firstLetterUppercase = (word) => {
  return (
    word.substring(0, 1).toUpperCase() + word.substring(1, word.length + 1)
  );
};
...
const gridContainer = {
  ...
  hide() {
    searchedWordParagraph.innerText =
    ...
    definitions.forEach((def) => {
      ...
      synonyms.length > 0 &&
        synonyms.forEach((synonym) => {
          synonymsGroup.push(synonym);
        });

      const antonymsGroup = [];
      antonyms.length > 0 &&
        antonyms.forEach((antonym) => {
          antonymsGroup.push(antonym);
        });
      ...
      html += `
            ...
            <ul>
                <li>${
                  synonyms.length > 0
                    ? synonyms.join(", ")
                    : "No results for synonyms to this word"
                }</li>
            </ul>
            ...
            <ul>
                <li>${
                  antonymsGroup.length > 0
                    ? antonymsGroup.join(", ")
                    : "No results for antonyms to this word"
                }</li>
            </ul>
  ...
  let html = `
    <div class="container">
        ...
        <div class="info-content">
            <p>Phonetic: ${
              phonetic ?? "No results for the phonetic of this word"
            }</p>
        </div>
